...
int main() {
    ...
    for (size_t i = 0; i < GRID_SIZE_X; i++) {
        ...
        for (size_t j = 0; j < GRID_SIZE_Y; j++) {
            ...
            Position pos = {glm::vec4(PIXEL(((float)WORLD_WIDTH / GRID_SIZE_X) * CELL_SIZE_X * i, WORLD_WIDTH),
                PIXEL(((float)WORLD_HEIGHT / GRID_SIZE_Y) * CELL_SIZE_Y * j, WORLD_HEIGHT), 0.0f, 1.0f)};
            Sprite rect_in = { {glm::vec2{PIXEL(((float)WORLD_WIDTH / GRID_SIZE_Y) * CELL_SIZE_Y, WORLD_WIDTH),
                               PIXEL(((float)WORLD_WIDTH / GRID_SIZE_X) * CELL_SIZE_X, WORLD_HEIGHT)}},
                   {glm::vec2{0, 0}},
                 0,
                 {glm::vec3{red, green, blue}}, false};
            auto ent = sprite2D(app, pos, rect_in);
            grid[i][j] = {ent, false};
        }
    }
    ...
    app.add_update_system([&grid] (App& app) {
            for (size_t i = 0; i < GRID_SIZE_X; i++) {
            for (size_t j = 0; j < GRID_SIZE_Y; j++) {
            auto &toDraw = app.ECS->reg.get<Sprite>(grid[i][j].entt).isRendered;
            if (grid[i][j].isShown) {
            toDraw = {true};
            }
            else {
    ...
    .add_fixed_update_system(  // Systems for updating particle physics
            ...
            for (size_t i = 0; i < GRID_SIZE_X; i++) {
                for (size_t j = 1; j < GRID_SIZE_Y; j++) {  // Last layer will not get update (for now)
                    if (grid[i][j].isShown) {
                        if (!grid[i][j - 1].isShown) {  // If there is no particle
                                                 // just beneath it
                            ...
                            if (int(i) + direction >= 0 &&
                                ...
                                if (!grid[i + direction][j - 1].isShown) {
                                    grid[i][j].isShown = false;
                                    grid[i + direction][j - 1].isShown = true;
                                }
                            }
                        }
                    }
            ...
            for (size_t i = 0; i < GRID_SIZE_X; i++) {
                for (size_t j = 0; j < GRID_SIZE_Y; j++) {
                    ...
                    if (distance < CELL_SIZE_X * brushRadius) {
                        ...
                        spdlog::debug("Putting pixel at {} {}", (WORLD_WIDTH / GRID_SIZE_X) * CELL_SIZE_X * i, (WORLD_HEIGHT / GRID_SIZE_Y) * CELL_SIZE_Y * j);
                    }
                }
            }
        }
    ...
    .add_update_system([](App& app) {
        ...
        std::min(brushRadius, 100.0f));  // Adjust the maximum
